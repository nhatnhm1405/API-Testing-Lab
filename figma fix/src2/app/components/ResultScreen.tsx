...
import { Zap, Star, RotateCcw, ArrowRight } from "lucide-react";
...
export function ResultScreen({ onNavigate, score, total, xpEarned, streak }: ResultScreenProps) {
      ...
      <div style={{ maxWidth:480,width:'100%',display:'flex',flexDirection:'column',alignItems:'center' }}>
        ...
        <motion.div initial={{ opacity:0,scale:.9 }} animate={{ opacity:1,scale:1 }} transition={{ duration:.4,delay:.5,type:'spring',stiffness:400,damping:28 }}
          ...
          <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:12 }}>
            {[
              { icon:<Star size={15} color="#F59E0B" fill="#F59E0B"/>, label:'Điểm số',      value:`${pct}%`,       color:'#D97706' },
              { icon:<Zap  size={15} color="#B9E534" fill="#B9E534" strokeWidth={0}/>, label:'XP kiếm được', value:`+${displayXP}`, color:'#5A700A' },
              { icon:<span style={{ fontSize:14 }}>🔥</span>,             label:'Chuỗi ngày',  value:`${streak}`,     color:'#EA580C' },
            ].map(st => (
        ...
        <motion.div initial={{ opacity:0,y:12 }} animate={{ opacity:1,y:0 }} transition={{ duration:.4,delay:.65 }}
          ...
          <div>
            <p style={{ fontFamily:'var(--atl-font-body)',fontSize:'14px',fontWeight:700,color:'#3730A3',margin:'0 0 2px' }}>Skill Check mở khóa!</p>
            <p style={{ fontFamily:'var(--atl-font-body)',fontSize:'12px',fontWeight:500,color:'#6366F1',margin:0 }}>Kiểm tra kiến thức module 1</p>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity:0,y:12 }} animate={{ opacity:1,y:0 }} transition={{ duration:.4,delay:.75 }}
          ...
          <TactileButton variant="ghost" size="md" onClick={() => onNavigate('lesson')} icon={<RotateCcw size={15}/>}>Làm lại</TactileButton>
          <div style={{ flex:1 }}>
